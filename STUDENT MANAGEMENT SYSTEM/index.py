...
#DEVELOPED BY Pawan Kumar(@itz_pa1)
...
def Database():
    ...
    cursor = conn.cursor()
    #cursor.execute("DROP TABLE member;") if you have edited this code once uncomment this code and then run .. Comment it again
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER NOT NULL  PRIMARY KEY AUTOINCREMENT, firstname TEXT, lastname TEXT, gender TEXT, age TEXT, course TEXT, address TEXT, contact TEXT)")
...
def SubmitData():
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or AGE.get() == "" or COURSE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "":
        result = tkMessageBox.showwarning('', 'Please Fill The Required Field', icon="warning")
    else:
        tree.delete(*tree.get_children())
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO `member` (firstname, lastname, gender, age, course, address, contact) VALUES(?, ?, ?, ?, ?, ?, ?)", (str(FIRSTNAME.get()), str(LASTNAME.get()), str(GENDER.get()), int(AGE.get()), str(COURSE.get()), str(ADDRESS.get()), str(CONTACT.get())))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `firstname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def UpdateData():
    if GENDER.get() == "":
       result = tkMessageBox.showwarning('', 'Please Fill The Required Field', icon="warning")
        ...
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        cursor.execute("UPDATE `member` SET `firstname` = ?, `lastname` = ?, `gender` =?, `age` = ?, `course` = ?,  `address` = ?, `contact` = ? WHERE `mem_id` = ?", (str(FIRSTNAME.get()), str(LASTNAME.get()), str(GENDER.get()), str(AGE.get()), str(COURSE.get()), str(ADDRESS.get()), str(CONTACT.get()), int(mem_id)))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `firstname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def OnSelected(event):
    ...
    #===================LABELS==============================
    lbl_title = Label(FormTitle, text="Updating Student Record", font=('arial', 16), bg="orange",  width = 300)
    ...
    lbl_course.grid(row=4, sticky=W)
...
def DeleteData():
    if not tree.selection():
       result = tkMessageBox.showwarning('', 'Please Select Desired Record First!', icon="warning")
    else:
...
def AddNewWindow():
    ...
    #==================BUTTONS==============================
    btn_addcon = Button(StudentForm, text="Save", width=50, command=SubmitData)
...
tree = ttk.Treeview(TableMargin, columns=("MemberID", "Firstname", "Lastname", "Gender", "Age", "Course", "Address", "Contact"), height=400, selectmode="extended", yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
